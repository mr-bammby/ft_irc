...
namespace Reply
{
	std::string	welcome(Client *user)
	{
		...
		std::string		test;
		test = "001" + user->getNickname() + ":Welcome to the Internet Relay Network" + user->getNickname() + "!" + \
		...
		**/
		return(Message(test, user).buildRawMsg());
	}
	...
	std::string motd(Client *user)
	{
		// Client server;
		std::string test;
		test = "372" + user->getNickname() + ": fancy cool ascii image plus rules list";
		// std::vector<std::string> params;
